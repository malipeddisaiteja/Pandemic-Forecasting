...
def upload(): 
    ...
    confirm = pd.read_csv('temp/confirm.csv', header=0, parse_dates=[0], index_col=0).squeeze("columns")
    death = pd.read_csv('temp/death.csv', header=0, parse_dates=[0], index_col=0).squeeze("columns")
    recover = pd.read_csv('temp/recover.csv', header=0, parse_dates=[0], index_col=0).squeeze("columns")
    text.delete('1.0', END)
    text.insert(END,filename+"Dataset uploaded\n");
...
def preprocess():
    ...
    print("c train: ", confirm_trainX)
    print("c test: ", confirm_testX)
...
def trainLSTM():
    ...
    confirm1 = pd.read_csv('temp/confirm.csv', header=0, parse_dates=[0], index_col=0).squeeze("columns")
    death1 = pd.read_csv('temp/death.csv', header=0, parse_dates=[0], index_col=0).squeeze("columns")
    recover1 = pd.read_csv('temp/recover.csv', header=0, parse_dates=[0], index_col=0).squeeze("columns")
    confirm1.fillna(0, inplace=True)
    death1.fillna(0, inplace=True)
    recover1.fillna(0, inplace=True)
    ...
    # Save the confirm1_model
    confirm1_model.save('confirm1_model.h5')
    ...
    # Save the death1_model
    death1_model.save('death1_model.h5')
    ...
    # Save the recover1_model
    recover1_model.save('recover1_model.h5')
...
def runLSTM():
    ...
    confirm1_model = load_model('confirm1_model.h5')
    death1_model = load_model('death1_model.h5')
    recover1_model = load_model('recover1_model.h5')
    ...
    rmse_confirm = mean_squared_error(confirm1_rawValues[-30:], confirm1_prediction_list, squared=False) / 10000
    rmse_death = mean_squared_error(death1_rawValues[-30:], death1_prediction_list, squared=False) / 10000
    rmse_recover = mean_squared_error(recover1_rawValues[-30:], recover1_prediction_list, squared=False) / 10000

    text.insert(END, '\nLSTM Confirm Root Mean Square Error: ' + str(rmse_confirm))
    text.insert(END, '\nLSTM Death Root Mean Square Error: ' + str(rmse_death))
    text.insert(END, '\nLSTM Recover Root Mean Square Error: ' + str(rmse_recover))

    lstm_error = lstm_error + (rmse_confirm + rmse_death + rmse_recover)/3
...
def graph():
    ...
    bars = ('SVM Error','Linear Regression Error','Lasso Error','ES Error','LSTM Error')
...
svmButton1 = Button(main, text="Run SVM Algorithm", command=runSVM, bg='#ffb3fe')
...
graphButton.config(font=font1)
'''graphButton = Button(main, text="Train LSTM", command=trainLSTM, bg='#ffb3fe')
